...
const Addcategory = (props) => {
  ...
  function handleSubmit(event) {
    ...
    axios({
      method: 'post',
      url: `${process.env.REACT_APP_API_URL}/category/addcat`,
      data: catInput,
      headers: {
    ...
    .then(function (response) {
      ...
      if(response.data && props.setCategories) {
        props.setCategories(prev => [...prev, response.data]);
      } else if (props.fetchCategories) {
    ...
    <div id="content" style={{ padding: '20px', backgroundColor: 'rgb(237, 229, 229)', boxShadow: '0px 2px 10px rgba(0, 0, 0, 0.1)' }}>
      <form autoComplete="off" onSubmit={handleSubmit}>
        ...
        <TextField
          ...
          onChange={(event) =>
            setcatInput({ ...catInput, category_name: event.target.value })
          }
        ...
        <TextField
          ...
          value={catInput.description}
          onChange={(event) =>
            setcatInput({ ...catInput, description: event.target.value })
          }
          fullWidth
          InputProps={{
            startAdornment: (
